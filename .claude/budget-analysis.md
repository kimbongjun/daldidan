...
| 순위 | 항목 | 상태 |
|------|------|------|
| 1 | 영수증 전용 스토리지 분리 (2-1) | ✅ 완료 |
| 2 | 월별 필터 + API 파라미터 (2-5) | ✅ 완료 |
| 3 | 전체 로드 → 월별 필터 (2-4) | ✅ 완료 |
| 4 | 삭제 확인 UX (2-7) | ✅ 완료 |
| 5 | 입력값 서버 검증 (2-8) | ✅ 완료 |
| 6 | OCR confidence 필터 (2-9) | ✅ 완료 |
| 7 | BUYER_OPTIONS 동적 설정 (2-6) | ✅ 완료 |
| 8 | 미사용 Server Actions 정리 (2-3) | ✅ 완료 |
| 9 | 카테고리 예산 한도 (2-11) | ✅ 완료 |
| 10 | 구매자 정산 기능 (2-13) | ✅ 완료 |
...
### OCR 추가 개선 여지
- `inferConfidence` 기반 저신뢰도 필드 제외 → ✅ 완료 (threshold 0.6, 30% fallback)
- NAVER CLOVA 특화 모델(영수증) 승인 후 Receipt 구조체로 전환 → 정확도 대폭 향상 예정

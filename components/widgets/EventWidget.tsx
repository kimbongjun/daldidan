    ...
    <div className="bento-card gradient-rose h-full flex flex-col p-5 gap-4">
      <div className="flex items-center justify-between">
        <div className="min-w-0">
          ...
          <h2 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>상영중 · 예매중 · 진행중</h2>
          <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>{source === "mixed-live" ? "실데이터 수집" : "샘플 데이터"}</p>
        </div>
          ...
          return (
            <Link key={item.id} href={`/culture/${item.slug}`} className="rounded-xl p-3 flex items-center gap-3 hover:opacity-80 transition-opacity" style={{ background: "rgba(255,255,255,0.04)" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs mt-0.5 truncate" style={{ color: "var(--text-muted)" }}>{item.venue}</p>
              </div>
